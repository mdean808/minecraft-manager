...
const sendDiscordMessage = async (title: string, description: string, color: number) => {
...
const stopEc2Instance = async () => {
  const res = await fetch('https://stopminecraftserver-ykuqto64rq-uc.a.run.app', { headers: {'authorization': process.env.AUTH} });
...
(async () => {
  ...
  setInterval(async () => {
    const { players, online } = await fetchServerInfo({ address: '127.0.0.1', port: 25565 });
    if (online) {
      if (players === 0) {
        sendDiscordMessage('Server Stop', 'Stopping server due to inactivity.', 1479403)
        //stop the server
        server.stdin.write('stop\r')
        // delay 50 seconds so the process stops correctly
        await delay(50000);
        server.kill('SIGINT');
        await delay(2000);
